...
When using the element for the xref:mod:engine:feature/contextMenu.adoc[Context Menu] this property is not required and actually ignored, because spawning from the Context Menu has it's own logic to determine the position for the object.
