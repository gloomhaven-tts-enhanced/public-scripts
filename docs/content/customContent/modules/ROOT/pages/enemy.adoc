...
Each Ability Card within the xref:_ability_deck[] needs to have a special naming structure.
...
local stats = {
  ...
  range = 0,
  -- A list of extra effect like retaliate or flying
  attributes = { },
...
The `attributes` field describes extra attributes for the enemy, like if it's flying or has retaliated damage.
It's a list of strings with a special (really awkward...) format.
The possible values are:

- Innate advantage: `"Advantage"`
- Innate disadvantage for attackers: `"Attackers gain Disadvantage"`
- Flying: `"%flying%"`
- Retaliate without range: `"%retaliate% 2"`
- Retaliate with range: `"%retaliate% 2: %range% 2"`
- Shield: `"%shield% 4"`
- Innate status effect: `"%muddle%"`, `"%poison%"`, `"%wound%"`, `"%immobilize%"`, `"%disarm%"`, `"%stun%"`, `"%strengthen%"`, `"%curse%"`,
- Multiple Targets: `"%target% 2"`
- Innate attack modifier: `%pierce% 2%"`, `%push% 2%"`
- Generate elements: `"%genAir%"`, `"%genEarth%"`, `"%genFire%"`, `"%genIce%"`, `"%genLight%"`, `"%genDark%"`, `"%genAny%"`,

Immunities have the same format.
Only the following values are currently supported:

- `"%muddle%"`, `"%poison%"`, `"%wound%"`, `"%immobilize%"`, `"%disarm%"`, `"%stun%"`, `"%curse%"`, `"%pull%"`, `"%push%"`

INFO::
The current definition for `attributes` and `immunities` is really awkward and limiting (e.g. it doesn't work for custom conditions).
This is due to the fact that this is inherited from Fantasy Setup.
This will change to an easier and more supportive syntax in a future version.
