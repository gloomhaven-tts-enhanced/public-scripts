...
| isUnlocked(string, string): boolean
| Takes the type and name of an unlockable campaign element (e.g. `campaign-sticker` and `Friends of the Fish King`) and returns `true` if this element is currently unlocked in the campaign and `false` otherwise.

| isRoomOpen(string): boolean
| Returns `true` if the room with the given index is open in the current running scenario and `false` if not.
|===
